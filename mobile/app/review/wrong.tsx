...
export default function WrongReviewScreen() {
  ...
  const review = useQuery({
    queryKey: queryKeys.review("wrong"),
    queryFn: async () => {
      const res = await reviewApi.get("wrong", 1, 50);
  ...
  return (
    <SafeScreen>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Text style={styles.subtitle}>Questions you answered incorrectly</Text>
        ...
        {(review.data || []).length === 0 ? (
          ...
          (review.data || []).map((q: any) => (
            <Card key={q._id} style={styles.card}>
              ...
              {q.text?.ml && <Text style={styles.questionTextMl}>{q.text.ml}</Text>}
              <View style={styles.meta}>
                <Text style={styles.metaText}>{q.topicId}</Text>
                <Text style={styles.metaText}>Answer: {q.correctOption}</Text>
              </View>
              {q.explanation?.en && (
                <Text style={styles.explanation}>{q.explanation.en}</Text>
              )}
            </Card>
          ))
        )}
...
const styles = StyleSheet.create({
  ...
  questionTextMl: { fontSize: 12, color: "rgba(226,232,240,0.5)", marginTop: 2 },
  ...
  explanation: { fontSize: 12, color: "rgba(226,232,240,0.6)", lineHeight: 18, marginTop: 8 },
});
