...
export default function FocusWeakPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setError(null);
      try {
        const res = await fetch("/api/focus/weak");
  ...
  if (error) {
    return (
      <div className="px-4 pt-6 pb-24">
        <Link href="/weak-areas" className="text-sm text-surface-200/60 hover:text-surface-200 transition-all">
          ← Weak Areas
        </Link>
